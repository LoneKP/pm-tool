class ProjectTimeFetcher
  ...
  def call
    TimeTracking.where(project_id: @project.id).delete_all
  ...
  def total_hours_billable
    billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def total_hours_design
    design_billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def total_hours_programming
    programming_billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def total_hours_project_management
    project_management_billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def total_hours_meetings
    meetings_billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def total_hours_daily_standup
    daily_standup_billable_time_entries_hours_per_day.map(&:sum).each_slice(1).to_a
  end
  ...
  def combine_project_management_and_meetings_hours
    project_management_hours.merge(meetings_hours) { |_key, oldval, newval| oldval.merge(newval) }
  end

  def combine_design_hours_and_programming_hours
    design_hours.merge(programming_hours) { |_key, oldval, newval| oldval.merge(newval) }
  end

  def combine_all_task_hours
    combine_design_hours_and_programming_hours.merge(combine_project_management_and_meetings_hours) { |_key, oldval, newval| oldval.merge(newval) }
  end

  def combine_with_billable_hours
    combine_all_task_hours.merge(billable_hours) { |_key, oldval, newval| oldval.merge(newval) }
  end

  def combine_all_hours
    combine_with_billable_hours.merge(daily_standup_hours) { |_key, oldval, newval| oldval.merge(newval) }
  end
end
